...
```
CLI 输出会附带只读「审计 / 地形 / 地图结构 / SLK / 游戏常数 / 脚本诊断 / 崩溃风险 / 秘籍口令 / 命令 / 资源 / 兼容」摘要，提示缺失地图信息、war3map.w3e 地形纹理与网格、地形中文名/贴图路径、区域/镜头/声音/路径图摘要与可读条目、内嵌 SLK 表行列摘要、war3mapMisc.txt 覆盖项、脚本缺失、异步/本地状态风险调用、已知崩溃配置、官方秘籍/调试口令残留、引用覆盖偏低、孤立对象比例偏高、重复命令串、脚本数值 Order ID 解码、自定义游戏平衡常数、未引用素材、1.20E return bug 迁移风险、1.24E 兼容风险等；这些提示只用于人工排查，不会修改地图。
...
- **地图信息**：独立标签页展示 war3map.w3i —— 真实地图名/作者/描述/推荐人数/尺寸/脚本语言(JASS/Lua)/各玩家(类型·种族·名字)/队伍(同盟·共享)；地图名优先取 w3i（比 HM3W 头权威，自动还原 TRIGSTR）。战役 `.w3n` 另解析 war3campaign.w3f 显示战役名/作者/难度/描述
...
- **资源依赖分析**（CLI/核心模块）：从对象字段、图标路径、脚本文本和内部文件清单提取模型/贴图/音频/字体引用，统计引用素材、内部素材和未引用素材，方便排查冗余导入或缺失引用链。
- **版本兼容报告**（CLI/核心模块）：默认按 1.24E 经典环境做静态检查，提示 Lua 脚本、1.31+ w3i 格式、大地图标志、DDS 贴图、1.20E 常见 `H2I`/`I2U` return bug 句柄转换等兼容风险；不做自动转换。
- **地形纹理摘要**（CLI/核心模块）：读取 `war3map.w3e` 头部，展示地形网格尺寸、基础地形集、自定义地形集标志、地表/悬崖纹理 ID，并把可识别地表纹理补全为编辑器中文名与 `TerrainArt\...\*.blp` 贴图路径，方便对照地形纹理备忘录做静态排查。
- **地图结构摘要**（CLI/核心模块）：读取 `war3map.w3r/.w3c/.w3s/.wpm`，展示区域、镜头、声音数量、路径图尺寸，并从结构文件中提取可读的区域名、镜头名、声音路径等零结尾字符串，补齐常用内部文件说明里的静态排查信息。
...
| `imp.py` | war3map.imp 导入文件清单解析(listfile 被删时补全可导出文件名) |
...
| `terrain.py` | 地形纹理摘要：解析 war3map.w3e 的地形集、纹理 ID 与网格尺寸 |
| `terrain_tiles.py` | 地形 tile ID → 编辑器中文名/贴图路径可读化 |
| `mapmeta.py` | 地图结构摘要：解析 war3map.w3r/w3c/w3s/wpm 头部统计 |
