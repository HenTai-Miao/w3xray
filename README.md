...
- **AI 质检（可选）**：顶栏「⚙ 设置」里配置 AI CLI（claude/codex/opencode，通用命令模板 + 预设；登录交给各 CLI 自己），「AI 质检」按钮会对当前地图产出诊断报告并让 AI 解释/标注异常。开发期另有「AI 归因 + 提补丁」流程把建议存到 `ai_suggestions/`（人工审阅后应用，**不自动改代码**）。机制是「工具自查 + AI 归因」，AI 不直接解析二进制、运行时不自改解析逻辑。
