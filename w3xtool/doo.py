...
from .doo_drops import DropCountError, flatten_drop_sets, read_drop_sets
from .doo_models import Doodad, Unit
...
def parse_doodads(data: bytes) -> list[Doodad]:
    ...
    if len(best) < count:
        print(
            "[doo] war3map.doo 两种布局均未完整解析，保留 %d/%d 个装饰物"
            % (len(best), count),
            file=sys.stderr,
        )
    return best
...
def _read_unit(r: _Reader, skin: bool, v7: bool = False) -> Unit:
    ...
    if nitems < 0 or nitems > _CAP:
        raise DropCountError("背包物品数", nitems)
    items = []
...
def parse_units(data: bytes) -> list[Unit]:
    ...
    if len(best) < count:
        print(
            "[doo] war3mapUnits.doo 两种布局均未完整解析，保留 %d/%d 个单位"
            % (len(best), count),
            file=sys.stderr,
        )
    return best
