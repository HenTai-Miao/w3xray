...
from .object_text_categories import categories_for_text_sections
from .object_text_names import (
    TextObjectSourceKind as TextObjectSourceKind,
    trusted_names,
    trusted_source_kind,
)
from .object_text_records import (
    TextObjectRecord as TextObjectRecord,
    merge_text_object_records as merge_text_object_records,
    text_object_record_sort_key,
)
from .textobj import looks_like_text_object, parse_text_objects
...
def collect_text_object_records(
    ...
    *,
    ...
    return tuple(sorted(records, key=text_object_record_sort_key))
    ...
    for index, block in archive.iter_blocks():
        ...
        if md is not None:
            head_result = read_component(
                ...
                "object-text",
                source_name,
                lambda: archive.peek_block(block, _ANONYMOUS_HEAD_BYTES)[
                    :_ANONYMOUS_HEAD_BYTES
                ],
                stage="probe",
            )
            ...
            try:
                head = archive.peek_block(block, _ANONYMOUS_HEAD_BYTES)[
                    :_ANONYMOUS_HEAD_BYTES
                ]
...
def _require_named_records(
    ...
    records = _records_from_payload(
        payload, source_name, source_kind, minimum_objects=1
    )
...
def _records_from_payload(
    ...
    *,
    ...
    objects = tuple(
        (obj_id, fields)
        for obj_id, fields in parse_text_objects(decode_warcraft_string(payload))
        if fields
    )
    if len(objects) < minimum_objects:
        return ()
    categories = categories_for_text_sections(source_name, source_kind, objects)
    return tuple(
        TextObjectRecord(
            category=category,
            obj_id=obj_id,
            fields=fields,
            field_sources={field_name: source_name for field_name in fields},
            source_name=source_name,
            source_kind=source_kind,
        )
        for (obj_id, fields), category in zip(objects, categories, strict=True)
    )
