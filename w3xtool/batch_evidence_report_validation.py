...
def validate_evidence_report_summaries(
    ...
    text_mismatch = _validate_current_text(directory, summary)
    if text_mismatch is not None:
        return text_mismatch
    return _validate_relation_completeness(directory, summary)
...
def _validate_current_text(
    ...
    for row in rows:
        if not _yes_no(row[_TEXT_CURRENT]):
            continue
        match _text_state(row[_TEXT_STATE]):
            case ObjectTextState.SOURCE_UNAVAILABLE:
                unavailable += 1
            case ObjectTextState.SOURCE_CONFLICT:
                conflict += 1
            case _:
                pass
...
def _validate_relation_completeness(
    ...
    for path, (header, column) in zip(paths, rows, strict=True):
        for row in read_report_rows(directory / path, header):
