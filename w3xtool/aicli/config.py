...
# 一律走 stdin：提示词不进命令行 → 不受命令行长度限制，也避免 .cmd 经 cmd /c 二次解析
# 导致的命令注入（地图名等不可信内容会进提示词）。某 CLI 若不读 stdin，用户可在设置里改。
PRESETS = {
    "claude": AIProfile(name="claude", command=["claude", "-p"], input_mode="stdin"),
    "codex": AIProfile(name="codex", command=["codex", "exec"], input_mode="stdin"),
    "opencode": AIProfile(name="opencode", command=["opencode", "run"], input_mode="stdin"),
}
