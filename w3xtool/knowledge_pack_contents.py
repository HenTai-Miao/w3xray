...
def _load_trigger_data(game_data_path: str | None) -> tuple[TriggerDataTable | None, str]:
    source = open_game_data_source(game_data_path)
    try:
        inventory_view = source.inventory_view.value if supports_inventory(source) else "unavailable"
