...
class IconResolver:
    def __init__(
        ...
        except Exception:  # noqa: BLE001 - invalid optional map keeps icon lookup usable.
            self.map = None
        ...
        for p in extra_paths or []:
            ...
            except Exception:  # noqa: BLE001 - skip unreadable optional campaign archives.
                continue
    ...
    def close(self):
        ...
        for arch in [self.map] + self.extra:
            if arch is not None:
                try:
                    arch.close()
                except Exception:  # noqa: BLE001 - close every remaining optional archive.
                    continue
        self.map = None
        self.extra = []
            ...
            if self.game_dir:
                for n in _GAME_MPQS:
                    ...
                    if os.path.exists(p):
                        ...
                        except Exception:  # noqa: BLE001 - skip invalid optional game archives.
                            continue
        return self._game
    ...
    def _load(self, path: str):
        ...
        for arch in archives:
            ...
            for c in cands:
                ...
                except Exception:  # noqa: BLE001 - one corrupt icon candidate must not stop lookup.
                    continue
        return None
