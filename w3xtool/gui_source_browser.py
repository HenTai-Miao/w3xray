...
class SourceBrowserMixin:
    ...
    def on_refresh_dir(self) -> None:
        directory = self._cur_dir.get(self.mode) or self._load_config().get(f"last_dir_{self.mode}")
    ...
    def _scan_dir(self, directory: str, mode: str) -> None:
        ...
        def work() -> None:
            if mode == "campaign":
                files = glob.glob(os.path.join(directory, "**", "*.w3n"), recursive=True)
                files = sorted(set(files), key=lambda path: os.path.getmtime(path), reverse=True)
                items = [{"path": path, "name": os.path.basename(path),
                          "loaded": False, "views": None} for path in files]
    ...
    def _refresh_left_list(self) -> None:
        ...
        for index, (path, name) in enumerate(self._dir_maps):
            ...
            entries.append(MapEntry(iid=iid, title=name, subtitle=os.path.basename(path),
                                    action="打开地图"))
    ...
    def _refresh_campaign_tree(self) -> None:
        ...
        for index, campaign in enumerate(self._dir_campaigns):
            if query and query not in campaign["name"].lower():
                continue
            parent_id = f"c{index}"
            self.map_list.insert("", "end", iid=parent_id, text=f" {campaign['name']}",
                                 open=bool(campaign["loaded"]))
            self._node_map[parent_id] = ("campaign", index)
            action = "查看子图" if campaign["loaded"] else "加载战役"
            entries.append(MapEntry(iid=parent_id, title=campaign["name"],
    ...
    def _append_campaign_children(self, parent_id: str, campaign, entries) -> None:
        if campaign["loaded"] and campaign["views"]:
            for index, (label, md) in enumerate(campaign["views"]):
                ...
                entries.append(MapEntry(iid=child_id, title=label, subtitle=md.name,
                                        action="切换子图", depth=1))
        else:
            self.map_list.insert(parent_id, "end", iid=f"{parent_id}_load", text="  （展开加载…）")
    ...
    def _open_node(self, node_id: str) -> None:
        ...
        if kind == "path":
            ...
            self._campaign_path = next(
                (campaign["path"] for campaign in self._dir_campaigns
                 if campaign["views"] and any(md is payload for _, md in campaign["views"])), None)
    ...
    def _set_campaign_views(self, views, path=None) -> None:
        ...
        if views:
            ...
            entry = next((campaign for campaign in self._dir_campaigns
                          if campaign["path"] == path), None)
            if entry:
                entry["loaded"] = True
                entry["views"] = views
            else:
