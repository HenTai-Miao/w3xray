...
from w3xtool.script_function_index import build_script_function_index, format_script_function_index_tsv
...
class ScriptFunctionIndexTest(unittest.TestCase):
    def test_indexes_jass_functions_with_calls_codes_and_inbound_counts(self):
        ...
        self.assertIn("函数\t来源\t起始行\t结束行\t行数\t被调用次数\t内部调用数\t机制\t对象码\t调用函数\t摘要", text)
        self.assertIn("SaveHero\twar3map.j\t1\t4\t4\t1\t2\tHashtable:写整数; ObjectID:技能\tA001", text)
        self.assertIn("Init\twar3map.j\t6\t9\t4\t0\t2\tObjectID:单位\tH001\tCreateUnit; SaveHero", text)
    ...
    def test_indexes_lua_functions_and_ignores_comments(self):
        ...
        self.assertIn("SaveHero\twar3map.lua\t1\t5\t5\t1\t2\tPlatformSave:写服务器值", text)
