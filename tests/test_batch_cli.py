...
from tests.batch_cli_fixture import batch_cli_result
...
def test_run_batch_cli_returns_one_only_when_a_map_failed(
    ...
    state = BatchState(
        BATCH_SCHEMA_VERSION,
        (
            batch_cli_result(MapBatchState.COMPLETE),
            batch_cli_result(MapBatchState.FAILED),
        ),
    )
...
def test_run_batch_cli_uses_authoritative_publication_result(
    ...
    failed = replace(
        batch_cli_result(MapBatchState.FAILED), state=MapBatchState.PARTIAL
    )
    monkeypatch.setattr(
        batch_cli,
        "run_batch",
...
def test_first_sigint_requests_graceful_cancellation(
    ...
    def cancel_run(_options, *, cancellation, on_progress) -> BatchState:
        ...
        return BatchState(
            BATCH_SCHEMA_VERSION, (batch_cli_result(MapBatchState.CANCELLED),)
        )
...
def test_cli_progress_prints_counts_action_elapsed_rss_and_eta(
    ...
    def progress_run(_options, *, cancellation, on_progress) -> BatchState:
        ...
        return BatchState(
            BATCH_SCHEMA_VERSION, (batch_cli_result(MapBatchState.COMPLETE),)
        )
