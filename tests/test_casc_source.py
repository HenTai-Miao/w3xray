...
def _idx_row(encoding_key: bytes, archive_index: int, offset: int, size: int) -> bytes:
    ...
    return encoding_key[:9] + bytes([high]) + low.to_bytes(4, "big") + size.to_bytes(4, "little")


class CascSourceTest(unittest.TestCase):
    def test_native_casc_source_reads_unencrypted_blte_from_idx_and_data_archive(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            payload = b"ID;name\nhfoo;Footman\n"
            blte = _blte(payload)
            block = hashlib.md5(blte).digest() + (len(blte) + 30).to_bytes(4, "little") + (b"\0" * 10) + blte
            offset = 32
