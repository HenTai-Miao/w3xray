...
| Phase 45: AI CLI 自改进集成(brainstorm→Phase1) | complete | 先 brainstorming 定 4 决策(面向开发+用户/机制=工具自查+AI归因/接入=通用模板+3预设/自改=AI提补丁人工应用),写 spec(docs/superpowers/specs)。**后端(Phase 1a)**:aicli/runner.py 通用命令模板调 AI CLI(arg/stdin/file,超时/退出码/缺命令优雅降级)+ aicli/config.py(claude/codex/opencode 预设+JSON存取)+ audit.py(结构化诊断:文件在档但0对象/名字质量/乱码/旗标+report_text)+ aicli/improve.py(诊断→组提示→调AI→存ai_suggestions 或 返回标注)+ prompts/(attribution/annotate)。**GUI(Phase 1b)**:顶栏「⚙设置」对话框(管理AI profile/选当前/测试连通)+「AI质检」按钮。spec datas 打包 prompts/,improve 兼容 _MEIPASS。全 TDD,测试82→108全过 |
| Phase 46: AI 集成 Phase 2(运行时增强+开发入口) | complete | 新增第4标签页「AI 质检」:结果写入独立面板(不再弹窗)+耗时/费用与"只标注不改码"说明。config 加 auto_audit 开关并持久化(设置存取时保留)。「解析后自动质检」勾选框→_render_map 末尾 opt-in 自动触发(静默填充不抢标签)。「AI 归因+提补丁(开发)」按钮→run_attribution 存到 ~/.w3xray/ai_suggestions/ 并打开目录(完成原始诉求"用AI优化逻辑写入md")。GUI 冒烟测试覆盖标签/开关/无图安全。测试108→112全过 |
| Phase 47: 修 AI CLI 不可用(.cmd 包装) + 配置持久化 | complete | 实测「测试连通」全失败:claude/codex/opencode 是 npm/nvm 的 .cmd/.ps1 包装脚本(C:\nvm4w\nodejs\claude.cmd),裸名交 subprocess 报"命令未找到"。runner 加 _resolve_launch:shutil.which 按 PATHEXT 找到 .CMD,.cmd/.bat 走 cmd /c;不走 stdin 时给 DEVNULL 免 claude 空等3秒。实测真 claude arg/stdin 均返回。claude 预设改 stdin(大提示词不受命令行长度限制)。**持久化**:设置对话框关窗静默保存当前表单(编辑不丢)+ _on_close 落盘 AI 配置,下次启动自动恢复。测试112→114全过 |
| Phase 49: AI 全自动优化(无人工审阅) | complete | 用户改主意:不要"AI 提补丁等我审阅",要 AI 完全把控、直接改码。新增 aicli/autopt.py:run_auto_optimize 让 AI 读仓库源码+诊断→产出统一 diff→**自动 git apply→跑全套测试→全过才 commit+push,否则 git reset --hard+clean 回滚到改前**(测试门禁+失败回滚=唯一安全网,坏改动绝不进库;运行前要求工作区干净保护未提交改动)。extract_diff 抽围栏/裸 diff;find_repo 仅源码下可用。GUI「AI 自动优化(开发)」按钮+二次确认。8 个 autopt 单测(真实临时 git 仓库验证 过→提交/败→回滚/打不上→回滚/脏→中止/空 diff)。实测真 claude 跑 autopt 提示+健康图→正确判无需改→no-diff(整链路安全验证)。测试119→128全过 |
| Phase 48: superpowers 评审(2 reviewer 并行)+ 修缺陷 | complete | 派 2 个 code-reviewer 并行审 AI 集成与解析核心。修:**【严重】命令注入**——.cmd 经 cmd /c 二次解析,arg 模式下地图名(不可信)里的 "&\| 可执行任意命令→runner 加安全护栏拒绝 cmd/c+arg 组合,三预设全改 stdin(用户数据不进命令行);**【重要】自动质检在战役子图每次切换都重烧 AI**→_render_map 加 fresh 参数,仅真·新打开图才触发;**【重要】质检重复解析地图**→audit_loaded 复用内存 MapData,GUI 走 ai_annotate/ai_attribute(diag) 不再 load_map;**【重要】Huffman 零强制覆盖**→加 fuzz/封顶 smoke 测试(任意输入不崩不超 out_size);**【次要】wts 字节级找 '}' 会误伤 GBK 尾字节0x7D**→闭合括号锚定行首;GUI 测试 hermetic(打桩 config 读写不碰真实 ~/.w3xray);修 ResourceWarning。测试114→119全过 |
| Phase 50: autopt 评审加固(3 重要安全缺口) | complete | reviewer 专审 autopt 找出 3 个破坏"安全契约"的问题并修:(1)**并发**:按钮无防抖,双击起并发 git 操作互相破坏门禁不变量→GUI 加 _ai_busy 标志守住所有 AI 按钮;(2)**误推主干**:push 在 main/master 追踪远程的 checkout 会直推→改为 main/master 绝不自动推、其余 git push origin HEAD 显式推当前分支;(3)**门禁可被绕过**:AI 自己的 diff 能改 tests/ 让坏改动"过测试"→拒绝触碰 tests//pyproject/.spec/.gitignore 的 diff(_forbidden),提示词同步禁止;另:extract_diff 截掉裸 diff 尾部解释、commit 失败也回滚、面板展示改动文件清单(透明)。新增 4 测试。测试128→132全过 |
