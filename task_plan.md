...
| Phase 45: AI CLI 自改进集成(brainstorm→Phase1) | complete | 先 brainstorming 定 4 决策(面向开发+用户/机制=工具自查+AI归因/接入=通用模板+3预设/自改=AI提补丁人工应用),写 spec(docs/superpowers/specs)。**后端(Phase 1a)**:aicli/runner.py 通用命令模板调 AI CLI(arg/stdin/file,超时/退出码/缺命令优雅降级)+ aicli/config.py(claude/codex/opencode 预设+JSON存取)+ audit.py(结构化诊断:文件在档但0对象/名字质量/乱码/旗标+report_text)+ aicli/improve.py(诊断→组提示→调AI→存ai_suggestions 或 返回标注)+ prompts/(attribution/annotate)。**GUI(Phase 1b)**:顶栏「⚙设置」对话框(管理AI profile/选当前/测试连通)+「AI质检」按钮(对当前图跑标注弹窗,后台线程不卡UI)。spec datas 打包 prompts/,improve 兼容 _MEIPASS。全 TDD,测试82→108全过。Phase 2(运行时增强)已含基础(AI质检按钮);后续可深化 |
